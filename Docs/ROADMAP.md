...
- [ ] SCP read/write (D-007); optional read-only IPF behind a licence-gated feature flag (C-003). **No material**: there is not one `.scp` or `.ipf` file to work from, which is D-009's position for DMS.
