...
- [ ] **Clock-bit verification.** Clock bits are currently masked off rather than checked, so MFM encoding violations — themselves a protection technique — cannot be detected (SPEC §What this decoder does not check).
