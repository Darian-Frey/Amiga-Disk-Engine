...
Phases 0 and 1 are complete. Phases 2–5 are each partly delivered, and **as of 2026-08-29 nothing unblocked remains**: what is left is blocked on material or hardware, or settled by a decision — DMS (D-009), LNFS (D-013), virus signatures (D-014), F-012 undelete (no deleted headers in 90 corpus disks), F-006 and F-005 (no Greaseweazle board), F-017 (cut, D-017), WHDLoad and OpenRetro (D-016). The register has no undecided entries. SCP reading and F-018's GUI half both landed 2026-08-28. Nothing unblocked remains in Phases 2–5; what is left waits on material or hardware.
   ...
   **The block map colours files; the hex pane does not.** Not an inconsistency: under text being read, tinting four fifths of a disk distinguishes nothing, and on a map every cell is a block so leaving files blank leaves it empty. Both palettes are derived from the widget's own `QPalette::Base`, and the file-and-empty mix was **solved for contrast** — the first attempt put empty space at 1.16 against the page, where "empty" and "past the end of the disk" are the same picture. Measure a colour choice like this; it is not visible by eye until somebody uses it.
   **Size alone is not evidence of a container.** The sniffing cascade's fallback called anything that was not an exact floppy geometry a hardfile, so an Amiga executable opened as a damaged 5,732-byte hard disk (BUG-010). A hardfile is a raw volume and begins `DOS`; without that it is `Kind::Unknown`. A front end asks `ade_image_recognised` — "no kind of disk image" and "recognised but holds no volume" are different answers, and only the first should be declined.
   **An extra-cylinder disk is not a larger volume — a volume can be smaller than the file holding it.** Five of the six non-standard floppy-sized corpus images keep their rootblock at **880**; none has one at the midpoint of the file's own block count. Computing it from the file's length made them unmountable (BUG-009, fixed 2026-08-31). `volume_geometry` tries the file's geometry first and then the standard shapes **largest first** — on an HD disk block 880 is ordinary data, so a DD candidate tried first would claim an oversized HD image. A file shorter than any standard geometry is not rescued: it does not contain a whole volume. It is also why `ade create` has no cylinder count.
   **`DOS\0`–`DOS\7` is the whole AmigaDOS set and `ade create` writes six of them.** `DOS\6`/`DOS\7` (LNFS) are refused by name with the reason, per D-013. A blank `DOS\4`/`DOS\5` needs **no** dircache block — the pointer is zero and ADFlib mounts it, which is measured, not assumed.
