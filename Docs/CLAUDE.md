...
- **Code:** 12 crates, an `ade` CLI, a C ABI (`bridge/`) and a Qt6 GUI (`gui/`). `ade` does `info`, `check`, `ls`, `extract`, `convert`, `create`, `scan`, `find`, `layout`, `formats`, `batch`, `identify`, `diff` and `consolidate`; the GUI browses several images at once, previews, extracts by drag, and searches across all of them. `ade-flux` reads SCP captures as of 2026-08-28. Still unread: DMS (D-009) and IPF (C-003), both sniffed and honestly refused.
