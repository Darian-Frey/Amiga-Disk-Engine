...
**Outstanding:** the ManifeST contract (future `VOCABULARY.md`), and identification on *open* rather than on demand — today it is a command, not something the open path does. Both are ADE's own work rather than someone else's data.
